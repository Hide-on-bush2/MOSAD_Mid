...
//            NSLog(@"text = %@", text.text);
    ...
    //    }];
        ...
        [self.VC presentViewController:alert animated:YES completion:nil]; //以模态弹出的方式让弹出框出现
...
//            [self.commentList addObject:comment_info];
            ...
            if(![replies isKindOfClass:[NSNull class]]){
                for(int i = 0;i < replies.count;i++){
                    ...
                    CommentInfo* comment_info = [[CommentInfo alloc]initWithFrom:reply[@"User"][@"Name"] To:reply[@"Father"][@"Name"] Content:reply[@"Reply"][@"Content"] ID:reply[@"Reply"][@"ID"]];
                    [self.commentList addObject:comment_info];
                }
            }
            [self.commentView reloadData];
