...
- (void)setupUI{
    ...
    _flagLabel = [GlobalUI createLabelFont:14 titleColor:[UIColor whiteColor] bgColor:[UIColor greenColor]];
    _flagLabel.text = @"顶";
...
- (void)layoutSubviews{
    ...
    _flagLabel.frame = CGRectMake(18, CGRectGetMaxY(_avatorImg.frame) + 10, 30, 30);
    _flagLabel.clipsToBounds = YES;
    _flagLabel.layer.cornerRadius = 5;
    _titleLabel.frame = CGRectMake(CGRectGetMaxX(_flagLabel.frame) + 13, CGRectGetMinY(_flagLabel.frame), CGRectGetWidth(self.contentView.bounds) - CGRectGetMaxX(_flagLabel.frame) + 13, 40);
    
    CGFloat contentHeight = [_contentLabel.text heightWithWidth:CGRectGetWidth(self.contentView.bounds) - 40 font:14];
    _contentLabel.frame = CGRectMake(20, CGRectGetMaxY(_flagLabel.frame) + 20, CGRectGetWidth(self.contentView.bounds) - 40, contentHeight);
...
- (void)setModel:(CellModel *)model{
    ...
    if (_imgArray.count && model.imgsss.length > 0) {
    ...
    _commentLabel.text = [NSString stringWithFormat:@"%ld",model.comemtNum];
    _zanLabel.text = [NSString stringWithFormat:@"%ld",model.zanNum];
